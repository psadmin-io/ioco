...
def __firewall_pia():
    ...
    try:
        subprocess.run(["sudo","firewall-cmd", "--permanent", "--add-port=" + str(this.config.get('pia_port')) + "/tcp"], check=True)
    except:
...
def __firewall_db():
    ...
    try:
        subprocess.run(["sudo","firewall-cmd", "--permanent", "--add-port=" + str(this.config.get('db_port')) + "/tcp"], check=True)
    except:
